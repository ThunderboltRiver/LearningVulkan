...
namespace Tutorial::Graphics {
    uint32_t SwapChainVulkanSurfaceExtentPreference::clampToRange(uint32_t value, uint32_t min, uint32_t max) const {
        return std::max(min, std::min(max, value));
    }

    SwapChainVulkanSurfaceExtentPreference::SwapChainVulkanSurfaceExtentPreference(FrameBufferSize frameBufferSize):
        _frameBufferSize(frameBufferSize) {
    }

    VkExtent2D SwapChainVulkanSurfaceExtentPreference::chooseSatisfiedOne(VkSurfaceCapabilitiesKHR surfaceCapabilities) const {
